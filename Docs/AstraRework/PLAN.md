...
4. Redesign the entire architecture/storefront/entrance/zoning/customer flow now. The existing authored machine defaults fail validation. A measured 24 m² starter must contain checkout/laptop, basic cracking and finite receiving, with later equipment earned and placed. Resolve safe save migration and actual public/staff routes before the Blender asset manifest. The draft bench stays parked. Geometry timeout belongs to its geometry/performance pass; preserve sample/assertion quality.
